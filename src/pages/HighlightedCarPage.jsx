...
const HighlightedCarPage = () => {
    ...
    const handleHighlight = (car) => {
        if (!highlightedCars.some(c => c.brand === car.brand && c.model === car.model)) {
    ...
    const handleRemoveHighlight = (car) => {
        const newHighlightedCars = highlightedCars.filter(c => c.brand !== car.brand || c.model !== car.model);
        setHighlightedCars(newHighlightedCars);
        localStorage.setItem('highlightedCars', JSON.stringify(newHighlightedCars));
    };
    ...
    return (
        <Container>
            ...
            <Link to="/">
                <Button variant="secondary" className="mt-3">Return to Dashboard</Button>
            </Link>
        </Container>
    );
};
